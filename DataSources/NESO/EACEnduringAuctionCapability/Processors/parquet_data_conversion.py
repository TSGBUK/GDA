...
def convert_csv_to_parquet() -> None:
    ...
    def process_file(file_name: str) -> None:
        ...
        try:
            ...
            log(f"[work] retry latin1 {file_name}")
            df = pd.read_csv(
                csv_path,
                on_bad_lines="skip",
                engine="python",
            ...
            log(f"[error] Failed reading {file_name}: {exc}")
            return

        if df.empty:
            log(f"[skip] {file_name} has no rows")
            return
        ...
        try:
            ...
            if dropped:
                log(f"[clean] Dropped {dropped} empty rows in {file_name}")
