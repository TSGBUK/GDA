#!/usr/bin/env python3
"""Train a DemandData expert model from parquet data.

The model is intentionally simple and robust:
- Input: time/settlement features from DemandData parquet records
- Target: all numeric demand columns except explicit metadata fields
- Model: multi-output RandomForestRegressor (CPU) or cuML per-target RF (CUDA)
...
EXCLUDE_COLUMNS = {"SETTLEMENT_DATE", "SETTLEMENT_PERIOD", "DatetimeUTC", "year"}
...
def load_demand_parquet(parquet_dir: Path) -> pd.DataFrame:
...
def ensure_datetime(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "DatetimeUTC" in out.columns:
        ...
        date_series = pd.to_datetime(out.get("SETTLEMENT_DATE"), utc=True, errors="coerce", dayfirst=True)
        period_series = pd.to_numeric(out.get("SETTLEMENT_PERIOD"), errors="coerce")
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features["settlement_period"] = (
        pd.to_numeric(df.get("SETTLEMENT_PERIOD"), errors="coerce").fillna(0).astype(np.int16)
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Train DemandData expert model from parquet")
    parser.add_argument(
        "--parquet-dir",
        default=str(project_root() / "DataSources" / "NESO" / "DemandData" / "Parquet"),
        help="Path to DemandData parquet directory",
    )
    parser.add_argument(
        "--output-dir",
    ...
    print(f"[load] Reading demand parquet dataset from: {parquet_dir}")
    df = load_demand_parquet(parquet_dir)
    if df.empty:
    ...
    print_training_summary("Demand expert", actual_backend, metrics, performance, weights)

    output_dir.mkdir(parents=True, exist_ok=True)
    model_path = output_dir / "demand_expert_model.joblib"
    metrics_path = output_dir / "demand_expert_metrics.json"
