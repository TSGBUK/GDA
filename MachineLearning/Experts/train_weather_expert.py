...
TARGET_COLUMNS = ["Temperature_C", "Wind_Speed_100m_kph", "Solar_Radiation_W_m2"]
...
def ensure_datetime(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.dropna(subset=["Date", *TARGET_COLUMNS]).sort_values("Date")
    return out
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Train Weather expert model from parquet")
    parser.add_argument("--parquet-dir", default=str(project_root() / "DataSources" / "Weather" / "Parquet"))
    parser.add_argument(
        "--output-dir",
    ...
    if not parquet_dir.exists() or not list_parquet_files(parquet_dir):
        print(f"[info] Weather parquet not ready: {parquet_dir}")
        return 0
    ...
    print(f"[load] Reading weather parquet dataset from: {parquet_dir}")
    df = load_weather_parquet(parquet_dir)
    if df.empty:
    ...
    print(f"[train] Training weather expert with {len(df):,} rows on backend: {backend}")
    ...
    print_training_summary("Weather expert", backend, metrics, performance, weights)

    model_path = output_dir / "weather_expert_model.joblib"
    metrics_path = output_dir / "weather_expert_metrics.json"
